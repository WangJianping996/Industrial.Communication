...
本文面向类库使用者，集中说明 `Industrial.Communication` 的安装方式、主要功能、常用 API、协议地址规则、示例代码和生产注意事项。当前包版本为 `0.1.0`。
...
```powershell
dotnet add package Industrial.Communication.Protocols.S7 --version 0.1.0
```
...
```powershell
dotnet add package Industrial.Communication.Protocols.Mc --version 0.1.0
```
...
```powershell
dotnet add package Industrial.Communication.Protocols.OpcUa --version 0.1.0
```
...
```powershell
dotnet add package Industrial.Communication.Transports --version 0.1.0
```
...
```powershell
dotnet add package Industrial.Communication.Adapters --version 0.1.0
```
...
```powershell
dotnet add package Industrial.Communication.DependencyInjection --version 0.1.0
```
